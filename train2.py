...
def concatenate_side_by_side(list_images):
    return np.expand_dims(np.concatenate(list_images, axis = 2), axis = 0)
...
def validation(valloader, model, interp_target, writer, i_iter):
    mIoU = 0
    for index, batch_val in enumerate(valloader):
        image_val, label_val, _, name_val = batch_val
        ...
        label_val_col = colorize(np.squeeze(label_val, axis = 2), args.num_classes, palette, args.ignore_label)     
        output = np.expand_dims(output, axis=0)
        image_val = image_val.numpy()
        image_val = image_val[:,::-1,:,:]

        IoU = miou(output, label_val, n_classes = args.num_classes, ignore_classes = [0])
        mIoU = mIoU + IoU
    mIoU = mIoU / (args.num_val_images)

    # Save images for tensorboard
    image_to_save = np.squeeze(concatenate_side_by_side([np.squeeze(image_val, axis = 0), label_val_col, output_col]), axis = 0)
...
def main():
    ...
    for i_iter in range(args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            while True:
                try:
                    _, batch = next(trainloader_iter) # Cityscapes, only discriminator1
            ...
            D_out1 = model_D1(F.softmax(pred_target1)) # prediction from discriminator 1
