...
# LIST OF CHANGES:
#
# 1. Uncommented valloader                                               * EASY TO FIND THIS ERROR
# 2. Included --data-dir_val in the terminal command                     * MEDIUM
# 3. Fixed IMG_MEAN in the valloader DataLoader (IMG_MEAN_SOURCE)        * EASY
# 4. Fixed IMG_MEAN in the trainloader DataLoader (It should be IMG_MEAN_SOURCE not IMG_MEAN_SOURCE2) * EASY
# 5. Fixed palette (20 colors instead of 5)                              * EASY
# 6. Fixed val list text files (directories should not start with /)     * HARD TO FIND THIS ERROR
# 7. Fixed self.id_to_trainid in val and source dataset python files     * THIS WAS SUPER HARD TO FIND!
# 
# TIME TO FIX EVERYTHING: 2h11min
######################################################################################################
######################################################################################################
######################################################################################################
######################################################################################################
...
lr=args.learning_rate, momentum=args.momentum,weight_decay=args.weight_decay)
    ...
    for i_iter in range(args.num_steps):
        ...
        print(i_iter, args.num_steps, loss)
        ...
        if (i_iter % args.val_every== 0 and i_iter != 0) or i_iter == 1:
            validation(valloader, model, interp, writer, i_iter, [37,41,10])
